...
class PythonIntro(object):
    ...
    def datatypes(self, var_tup=None):
        ...
        for svalue in svalues:
            if svalue in var_tup:
                print('{} {} in var_tup gefunden.'.format(svalue, type(svalue)))
                exists = True
                break
            else:
                exists = False
        ...
        print 'Es ist ein Vorteil, dass strings veraenderbar sind, weil Pytho'\
            + 'n Pointer erstellt. Somit wuerde eine Veraenderung eines kopie'\
            + 'rten Strings auch den original string veraendern. Dies ist ins'\
            + 'besondere problematisch, falls Variablen als Dict-Keys verwend'\
            + 'et werden, die dann im Nachhinein durch Pointer-Veraenderung f'\
            + 'alsch sind.'
    ...
    def sequences(self, seq_start=3, seq_end=7, seq_step=1):
        ...
        mod_test_list = [ 
            ...
            test_list[:int(len(test_list))/3],
            test_list[int(len(test_list))/3:2*int(len(test_list))/3],
            test_list[2*int(len(test_list))/3]
            ]
        ...
        for index, element in enumerate(test_list):
            stringlist.append('%d: %d ' %(index, element))
        ...
        return mod_test_list, stringlist
    ...
    def sequences_complex(self, test_seq=(1, 1.0, '1.0')):
        ...
        datastring = []
        for x in test_seq:
            datastring.append('<{}, {}>,'.format(type(x).__name__, str(x)))
        ...
        ndatastring = []
        for x in test_seq:
            ndatastring.append('<%s, %s>' %(type(x).__name__, str(x)))
        return ', '.join(ndatastring)
    ...
    def list_comprehension(self, test_seq=(1, 1.0, '1.0')):
        ...
        test_typename = [type(x).__name__ for x in test_seq]
        tuple_typename = []
        for (x, y) in zip(test_seq, test_typename):
            tuple_typename.append('<{}, {}>'.format(x,y))
        string_typename = ', '.join(tuple_typename)
        print(string_typename)
        ...
        return (type(x).__name__ for x in test_seq)
    ...
    def dictionaries(self, rand_list=None):
        ...
        for x in range(min(rand_list), max(rand_list)):
            hist[x] = rand_list.count(x)
        
        print sorted(hist)
        histtuple = hist.items()
...
class NumPyIntro(object):
    ...
    def arrays(self, test_arr=None):
        ...
        arr = np.reshape(np.arange(5,17), (4, 3))
        ...
        arr_lin_rows = np.reshape(test_arr, -1, order=0)
        arr_lin_cols = np.reshape(test_arr, -1, order=1)
    ...
    def array_slicing(self, seq_arr=None):
        ...
        print(oben_m, vierte_m, odd_m)

        return oben_m, vierte_m, odd_m
    ...
    def array_indexing(self, seq_arr=None):
        ...
        eleme_array = seq_arr[[1,2,3],[3,1,1]]
        row_array = seq_arr[:,[2,3,6]]
        print eleme_array
        print row_array
        ...
        odd_array = seq_arr[seq_arr % 2 == 0]
        mask = np.logical_and(seq_arr % 3 == 0,seq_arr > 17)
        three_array = seq_arr[mask]
        
        # Speichern Sie die vier Egebnisse jeweils in ein Array und geben Sie diese
        # als Ergebnis der Funktion zurueck.

        return eleme_array, row_array, odd_array, three_array
    ...
    def array_operations(self, seq_arr=None):
        ...
        sum_matrix = seq_arr[0,:]+ seq_arr[1,:]
        prod_matrix = seq_arr[0,:]*seq_arr[1,:]
        skal_matrix = np.dot(seq_arr[0,:],seq_arr[1,:])
        ...
        return sum_matrix, prod_matrix, skal_matrix
    ...
    def array_functions(self, seq_arr=None):
        ...
        # mit dem Debugger.
        print '---------------------------------------------------'
        div_matrix = np.transpose(np.transpose(seq_arr) / sum_matrix)
        print div_matrix
    ...
    def array_distributions(self):
        ...
        round_arr_gaus = np.around(rand_arr_gauss,decimals=0)
        round_arr_unif = np.around(rand_arr_unif,decimals=0)
        reshaped_arr = np.reshape(round_arr_unif, -1)
        hist_gaus = np.bincount(round_arr_gaus.astype(np.int64))
        hist_unif = np.bincount(reshaped_arr.astype(np.int64))
        bar_plot(None, hist_gaus)
        bar_plot(None, hist_unif)
        ...
        sum_unif = np.sum(round_arr_unif, axis=1)
        hist_unif_sum = np.bincount(sum_unif.astype(np.int64))
        bar_plot(None, hist_unif_sum)
        return hist_unif_sum
